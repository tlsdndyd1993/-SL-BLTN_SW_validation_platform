# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v4.19
────────────────────────────────────────────────────────────────────────────
v4.18 → v4.19 수정사항

  [버그1 수정] 전원 조건 감시 ON 상태에서 조건 충족해도 타이머가 작동 안 하는 문제
    · 근본 원인 A (스레드 안전성):
        PowerController._fire_state_change()는 Arduino RX 백그라운드 스레드에서 호출됨.
        기존 _on_power_state_change()는 콜백 안에서 즉시 _eval_trigger() →
        _get_cond_rows_cached() → Qt 위젯 findChildren() 접근을 실행.
        Qt 위젯은 메인 스레드에서만 안전하게 접근 가능하므로
        백그라운드 스레드에서 접근 시 데이터 레이스 → 조건 평가 결과가
        무시되거나 크래시 없이 False 반환 → 타이머 작동 안 함.
    · 수정 (_on_power_state_change):
        QMetaObject.invokeMethod(..., Qt.QueuedConnection)으로 즉시 반환.
        실제 평가 로직을 @pyqtSlot(str, bool) _on_power_state_change_main()으로 분리.
        invokeMethod의 QueuedConnection이 메인 이벤트 루프에 작업을 큐잉하여
        메인 스레드에서 Qt 위젯에 안전하게 접근.
        최상단 QtCore import에 pyqtSlot, Q_ARG 추가.

    · 근본 원인 B (타이머 미시작):
        '전원 조건 감시' 토글 ON 시 _on_watch_toggled()에서
        _can_poll_timer.start()를 호출하지 않음.
        _can_poll_timer는 set_can_available(True) 호출 시에만 시작되므로
        CAN 연결 없이 전원 조건만 사용하는 경우 150ms 주기 평가가 전혀 실행 안 됨.
        → 전원 이벤트 콜백(_on_power_state_change)이 스레드 문제로 무시되더라도
          폴링으로도 복구가 안 되는 이중 장애.
    · 수정 (_on_watch_toggled):
        ON 시 _can_poll_timer.start(), OFF 시 _can_poll_timer.stop() 추가.
        CAN 연결 여부와 무관하게 전원 감시 ON이면 타이머가 항상 동작.

v4.17 → v4.18 수정사항

  [문제1 수정] 카메라 출력 프레임이 너무 낮음 (끊김 현상)
    · 근본 원인 A: _camera_loop()에서 time.sleep(interval) 후 cap.read()를 호출.
                   cap.read()는 내부적으로 grab+retrieve를 합쳐 실행하며
                   드라이버가 다음 프레임을 준비할 때까지 블로킹 대기를 함.
                   따라서 실제 루프 주기 = sleep(interval) + read() 블로킹
                   → interval만큼 추가 지연이 중첩되어 실제 FPS가 절반 이하로 떨어짐.
    · 근본 원인 B: CAP_PROP_FPS 반환값을 신뢰해 interval을 고정.
                   일부 드라이버는 실제 전송 가능한 FPS가 아닌 설정 요청값(30)을 그대로
                   반환하여 interval=1/30으로 설정되나 실제 카메라는 10fps 수준 →
                   read()가 3프레임에 1번만 새 프레임을 줘도 sleep이 먼저 끝나버려
                   낡은 버퍼 프레임이 반복 출력됨.
    · 수정 (_camera_loop):
        1) cap.read() → grab()/retrieve() 분리.
           grab()을 sleep 없이 최대한 빠르게 반복 호출(드라이버 내부 페이스 유지).
           retrieve()는 next_retrieve_t 주기가 됐을 때만 호출해 CPU 낭비 없이 최고 FPS.
        2) 2초마다 measured_fps(_cam_fps_ts)로 실측 FPS를 계산해
           actual_camera_fps와 interval을 자동 보정.
           드라이버 반환값과 실제 전송 FPS가 어긋나도 시간이 지나면 자동 수렴.

v4.16 → v4.17 수정사항

  [문제1 수정] 조건부 타이머 — CAN 조건 포함 시 타이머가 느리게 카운트되는 문제
    · 근본 원인:
        _eval_can_only()가 500ms QTimer(메인스레드)에서 호이될 때마다
        _eval_trigger → _eval_rows → findChildren(QWidget, "_cond_row") 실행.
        findChildren()은 Qt 위젯 트리를 재귀 탐색하는 비용이 큰 연산으로
        조건 행 수에 비례하여 메인 이벤트 루프를 ··· 점유.
    · 수정1: findChildren 결과를 _cond_row_cache에 캐싱
             (행 추가/삭제 시에만 캐시 무효화, 평가 시 캐시 재사용).
    · 수정2: _can_poll_timer 주기 500ms → 150ms 로 단축
             (캐싱으로 findChildren 비용 제거됐으로 더 자스 평가 가능).

  [수정1] 커넬 로그/결과 파일 저장 시 폴더 트리 반영
    · 근본 원인:
        _auto_export()에서 log_dir = base_dir/kernel_logs 로 단일 평탄 경로 사용.
        저장 경로 패널의 폴더 트리(vehicle_type/date/tc_id/extra_segments)가
        kernel_logs 아래에 반영되지 않아 다른 스크립트 결과가 섹임.
    · 수정: _auto_export()에서 engine의 vehicle_type, tc_id, extra_segments를 읽어
            kernel_logs/<vehicle_type>/<YYYYMMDD>/<tc_id>/<extra_segments...>/
            구조로 log_dir를 결정.
            _on_export_log() 수동 내보내기도 동일 경로 사용.

v4.15 → v4.16 수정사항

  [버그1 수정] _API_DOC 속성명 오류 — 커널 스크립트에서 임계값 설정 무효
    · 근본 원인: _API_DOC(블랙아웃 섹션)에 engine.blackout_threshold /
                 engine.blackout_enabled 로 잘못 문서화.
                 실제 CoreEngine 속성은 brightness_threshold / blackout_rec_enabled.
                 커널 스크립트에서 engine.blackout_threshold = 25 를 쓰면
                 새 임시 속성이 생성될 뿐 실제 임계값이 바뀌지 않아 동작 무효.
    · 수정: _API_DOC 블랙아웃 섹션 속성명 전부 실제 속성명으로 교정.
            _SYS_STATIC(AI Chat용 요약 doc)의 잘못된 속성명도 동일하게 교정.

  [버그2 수정] brightness_screen/camera_enabled DB 저장·복원 누락
    · 근본 원인: _save_settings() / _load_settings()에서
                 brightness_screen_enabled, brightness_camera_enabled 키가
                 완전히 누락 → 프로그램 재실행 시 항상 True로 리셋.
    · 수정: _save_settings()에 두 키 저장 추가.
            _load_settings()에 두 키 복원 추가 (default True).
            SettingsDB._init() CREATE TABLE은 key-value 방식이므로 스키마 변경 없음.

  [기능 추가] AI Chat 액션 3개 추가 — 미노출 설정값 제어
    · 근본 원인: blackout_cooldown / buffer_seconds / screen_fps 가
                 커널 스크립트에서는 직접 접근 가능하나 AI Chat 액션이 없어
                 대화형 제어 불가.
    · 수정: blackout_set_cooldown / set_buffer_seconds / set_screen_fps
            액션 핸들러 추가. _API_DOC에도 해당 액션 문서화.

v4.14 → v4.15 수정사항

  [수정1] 커널 실행 로그 오버레이 — 녹화 영상에 반영
    · 근본 원인: _apply_overlays()는 메모 오버레이만 적용하고 커널 로그는 미처리.
    · 수정 (CoreEngine._apply_overlays):
        KernelPanel의 _log_lines를 Engine이 직접 참조하는 대신,
        Engine에 kernel_log_overlay_lines: list, kernel_log_overlay_on_cam: bool,
        kernel_log_overlay_on_scr: bool 속성 추가.
        KernelPanel._on_log_safe()에서 overlay lines를 동기화.
        사이클 초기화 시(__CLEAR_KERNEL_LOG__) overlay lines도 함께 초기화.
        _apply_overlays()에서 source("screen"/"camera")와 overlay 활성 여부를
        확인해 draw_memo_overlay()로 커널 로그를 프레임에 합성.
    · UI: KernelPanel 로그 그룹 상단에 체크박스 2개 추가
        [□ 웹캠 오버레이]  [□ 디스플레이 오버레이]

  [수정2] ROI 밝기·OCR 값을 녹화 영상에 출력
    · 근본 원인: _draw_rois_on_frame()이 사각형·이름만 그리고
                 roi.last_brightness / OCR 텍스트를 누락.
    · 수정 (_draw_rois_on_frame):
        brightness_enabled(해당 소스) 조건 시 ROI 사각형 내부 하단에
        "L:xxx" 밝기값 표시.
        OCR enabled 조건 시 "T:xxx" OCR 텍스트 표시.
        (ROI 패널의 활성 OCR/밝기 플래그를 Engine 속성으로 전달)

v4.13 → v4.14 수정사항

  [문제1 수정] 스캔 버튼 클릭 시 프로그램 종료 (크래시)
    · 근본 원인: cv2.VideoCapture(idx, CAP_MSMF) 가 일부 PC/드라이버 환경에서
                 MSMF COM 객체 초기화 실패 시 UnhandledException 을 발생시켜
                 프로세스 전체를 강제 종료.
                 threading.Thread 로 격리해도 스레드 크래시 = 프로세스 종료이므로
                 스레드 격리만으로는 해결 불가.
    · 수정 (_do_scan_cameras):
        VideoCapture 탐색 코드를 별도 Python subprocess 로 격리 실행.
        subprocess 크래시 -> 부모 프로세스는 영향 없음.
        스캔 결과를 JSON stdout 으로 수신해 파싱.
        subprocess 실패/타임아웃(20초) 시 빈 결과로 안전 처리.
        subprocess 자체 실행 불가 환경 -> 인프로세스 폴백(_do_scan_cameras_inproc)
        사용. 결과 처리(camera_list 갱신, signals emit) 는 동일.

  [문제2 수정] 조건부 타이머 AND 다중 채널 조건 동작 안 됨
    · 근본 원인: _eval_one_row() 전원 조건 평가 로직:
                   return changed.get(ch, False) and curr.get(ch, False) == want
                 _on_power_state_change 콜백에서 changed 딕셔너리는
                 '이번 이벤트에서 변경된 채널' 만 True.
                 예) ACC OFF 이벤트 -> changed = {acc: True, ign: False, ...}
                 AND 조건에 [ACC OFF] + [IGN OFF] 두 행이 있으면:
                   acc 행: changed["acc"]=True, curr["acc"]=False -> True
                   ign 행: changed["ign"]=False -> 즉시 False
                 -> AND 전체 False. 두 채널이 동시 변경이 아닌 한 충족 불가.
                 시작/종료/랩 조건 모두 동일 함수 경유 -> 셋 다 영향.
    · 수정 (_eval_one_row, _on_power_state_change):
        전원 조건 행 평가에서 changed 조건 완전 제거.
        curr(현재 실제 상태)와 want(원하는 상태)만 비교:
          return curr.get(ch, False) == want
        트리거 시점 제어는 _on_power_state_change 콜백(이벤트 발생 시에만 호출)과
        _eval_can_only 주기 타이머가 이미 담당하므로 개별 행 평가는
        "현재 상태 == 원하는 상태" 만 판단하면 됨.
        _on_power_state_change 의 changed 딕셔너리도 all-False 로 교체
        (하위 호환 시그니처 유지, 실제 로직에선 미사용).
        시작/종료/랩 조건 모두 동일 _eval_one_row 경유 -> 셋 다 자동 수정됨.

v4.12 → v4.13 수정사항

  [문제1 수정] 카메라 FPS 낮음 (특히 노트북 내장 카메라 5~10fps)
    · 근본 원인 A: 기본 픽셀 포맷 YUY2 (비압축).
                   USB 대역폭 한계로 1280x720에서 최대 5~10fps 제한.
    · 근본 원인 B: FPS set() 이후 드라이버가 첫 read() 전까지 반영 안 하는 카메라 존재.
    · 수정 (_open_camera, _do_scan_cameras):
        1) CAP_PROP_FOURCC = MJPG 먼저 설정 -> 하드웨어 압축으로 대역폭 1/5 감소
           -> 내장 카메라 30fps 달성 가능.
        2) MJPG 미지원 카메라 -> YUY2 폴백 + 해상도 640x480으로 낮춰 FPS 확보.
        3) FPS 설정 후 dummy read() 3회 워밍업 -> 드라이버 파이프라인 즉시 반영.
        4) 상태바에 [MJPG/YUY2] 포맷과 실제 해상도, FPS 함께 표시.

  [문제2 수정] 타 PC에서 노트북 내장 카메라 출력 안 됨
    · 근본 원인: MSMF 백엔드가 일부 노트북 내장 카메라와 EXE 환경에서 충돌.
                 isOpened()=True 반환하지만 read()는 항상 실패하는 케이스 발생.
    · 수정 (_open_camera, _do_scan_cameras):
        1) open 직후 dummy read() 3회로 실제 프레임 수신 가능 여부 검증.
           실패 시 해당 백엔드 포기 -> 다음 백엔드(MSMF -> ANY) 시도.
        2) 스캔 시에도 동일 검증 적용 -> "목록엔 뜨지만 화면이 검은 것" 현상 차단.

v4.11 → v4.12 수정사항

  [플랫폼 수정 — fix4] blackout_rec_enabled=False 상태에서 ROI 밝기값 미갱신
    · 근본 원인: fix1에서 "blackout_rec_enabled=False이면 ROI 연산 전체 차단" 으로 수정했으나
                 커널 스크립트가 블랙아웃 OFF 상태에서 camera_overall_avg/screen_overall_avg를
                 읽으면 항상 이전 값(또는 np.zeros)이 반환되는 부작용 발생.
                 예) Step5: Key ON 후 ROI 밝기 감시 → blackout OFF 상태라 값이 0 고정 → FAIL 오탐.
    · 수정: ROI 밝기 계산(calc_roi_avg → roi_avg/overall_avg 갱신)은 blackout ON/OFF 무관하게
            _screen_loop/_camera_loop에서 항상 실행.
            블랙아웃 감지(_detect_blackout 호출)만 blackout_rec_enabled=True 일 때만 실행.
            prev 갱신도 비활성 상태에서 계속 수행 → 활성화 직후 첫 프레임 오탐 방지.

  [문제1 수정] '블랙아웃 클립 녹화 활성화' 비활성화 상태에서도 감지 횟수 카운트됨
    · 근본 원인: _detect_blackout() 진입 조건이
                 "not blackout_rec_enabled AND not tc_blackout_enabled" 였음.
                 → tc_blackout_enabled=True 이면 blackout_rec_enabled=False 여도
                   감지 연산·카운트·이벤트 emit이 모두 실행됨.
                 _screen_loop/_camera_loop 의 루프 진입 조건도 동일 문제.
    · 수정: _detect_blackout() 진입 조건을 "not blackout_rec_enabled" 단독으로 변경.
            스크린/카메라 루프의 ROI 밝기 계산 조건도 blackout_rec_enabled 단독으로 변경.
            → '블랙아웃 클립 녹화 활성화' 버튼이 꺼지면 카운트·이벤트·클립 저장 완전 차단.
            (tc_blackout_enabled 기능은 blackout_rec_enabled=True 가 전제이므로 동작 무변)

  [문제2 수정] 블랙아웃 녹화 영상에 감지 이전 구간(-20초)이 두 번 반복됨
    · 근본 원인: _save_bo_clip() 의 post 수집 루프에서
                 버퍼가 꽉 찬 경우(new_frames <= 0) fallback으로
                 list(buf)[-n_post:] 를 post로 사용 →
                 이 구간이 pre_clip 과 완전히 동일한 구간 → -20초 영상이 반복 재생됨.
    · 수정: snap_len(감지 직후 버퍼 길이) 기준으로 이후 새로 추가된 프레임만 post로 취득.
            버퍼 순환(cur_len < snap_len) 상황도 overflow 계산으로 방어.
            잘못된 fallback(기존 버퍼 재사용) 로직 완전 제거.
            수동녹화와 동일하게 "fps × 경과시간" 기반 elapsed_frames 방식으로 교체.
            ※ fix3에서 Queue 직접 주입 방식으로 재수정 (아래 참조)

  [추가 수정 — fix3] 블랙아웃 이후 구간이 저장되지 않는 문제 (재발)
    · 근본 원인: deque는 슬라이딩 윈도우. 어떤 polling 방식이든(snap_len 비교, fps×경과시간)
                 deque 끝 N개를 읽으면 20초 후에는 그 N개 안에 감지 이전 프레임이 섞임.
                 → 블랙아웃 감지 시점으로 계속 되돌아가는 현상.
    · 수정: CoreEngine에 _scr_bo_queues / _cam_bo_queues 레지스트리 추가.
            _screen_loop / _camera_loop에서 deque append 직후 등록된 Queue에 프레임 직접 put.
            _save_bo_clip은 Queue를 등록 후 감지 시점 이후 프레임만 정확히 수신.
            try/finally로 스레드 비정상 종료 시에도 Queue 등록 해제 보장.
...
"""
# ── OpenCV 내부 C++ 경고 콘솔 출력 억제 (VideoCapture DSHOW WARN 등) ──────
# cv2.setLogLevel(0) 은 Python 레이어만 억제 → 환경변수로 C++ 레이어까지 차단
import os as _os_pre
_os_pre.environ.setdefault("OPENCV_LOG_LEVEL", "ERROR")     # ERROR 미만 메시지 차단
_os_pre.environ.setdefault("OPENCV_VIDEOIO_DEBUG", "0")     # VideoIO 디버그 출력 차단
...
class Signals(QObject):
    ...
    manual_source_changed   = pyqtSignal(str)            # manual_source 변경 → UI 체크박스 동기화
    ...
    blackout_rec_changed   = pyqtSignal(bool) # blackout_rec_enabled 변경 시 UI 동기화용
    blackout_log           = pyqtSignal(str)  # 블랙아웃 감지 시 커널 로그 출력용
    kernel_log             = pyqtSignal(str)  # ★ 커널 로그 → 메인스레드 안전 전달
