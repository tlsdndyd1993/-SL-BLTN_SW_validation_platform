**업무 활성화용 단품 평가 환경 플랫폼 개발** 

-
(첨부파일)

-> 플랫폼 프로그램 : 증적 자료 생성(영상저장/이미지캡쳐), BLTN제어기 전원제어, CAN BUS 신호 read, T/C 스크립트 작성

-> Atmega328p아두이노 프로그램 : 전원 제어 모듈(BLTN B+ / CCU & HU B+ / ACC / IGN / PLBM실물&모사) ,  OHCL 입력  ,  OHCL LED PWM OUTPUT)
...
1. BLTN Input : 전원(BLTN B+, HU & CCU B+, ACC, IGN, PLBM 실물/모사) 제어 , OHCL 스위치 인가
...
[CAN]
1. (사전 조건) CANoe 실행
2. CANoe에 사용되고 있는 DB를 이 플랫폼에 업로드 후, 그래프 및 커널, Ai chat을 통해 CAN 값 READ.
3. CAN Graph UI 반영하여 간단한 동작 천이 확인 
-
[커널]
